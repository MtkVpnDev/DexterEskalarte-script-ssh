...
function InstApache(){
 ...
 wget -O /etc/apache2/sites-enabled/dextermtk.conf "https://raw.githubusercontent.com/MtkVpnDev/DexterEskalarte-script-ssh/main/dextermtk.conf"
 sed -i "s|yourserver|localhost|g" /etc/apache2/sites-enabled/dextermtk.conf
...
function InstPanel(){
 ...
 wget -O /home/panel/html/ypanel.zip https://www.dropbox.com/s/sps8as6kt0acmt6/Kobi_PanelFix.zip?dl=0
 sleep 2
...
read -p "Enter Script Password: " -e -i mtkdexter pwdo
if test $pwdo == "mtkdexter"; then
...
echo "" | tee -a log-install.txt
echo "YELLOW Script Installer by: DexterEskalarteTV"  | tee -a log-install.txt
echo "        (http://fb.com/eskalartedexter)         "  | tee -a log-install.txt
echo "" | tee -a log-install.txt
...
echo "" | tee -a log-install.txt
echo "[DONATION] GCash: 09709310250 Paypal: ennaojselarom@gmail.com " | tee -a log-install.txt
echo "" | tee -a log-install.txt
